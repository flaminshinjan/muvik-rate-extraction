...
class ValidatorPrompt extends __WEBPACK_EXTERNAL_MODULE__base_mjs_f55655df__.BasePrompt {
    tasksToValidate() {
        if (1 === this.tasks.length) return this.tasks[0];
...
`;
    ...
    getSystemMessage() {
        return new __WEBPACK_EXTERNAL_MODULE__langchain_core_messages_57fab301__.SystemMessage(`You are a validator of an agent who interacts with a browser.
YOUR ROLE:
...
${this.tasksToValidate()}`);
    ...
    constructor(task){
        super(), _define_property(this, "tasks", []);
        this.tasks.push(task);
    }
}
export { ValidatorPrompt };
